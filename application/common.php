...
function count_room_rent($roomid , $houseid = ''){
    ...
    if($roomOne['BanID'] == '1050053295' || $roomOne['RoomType'] == 12){ //如果是新华村5栋的楼，则单独处理
        ...
        //     }
        // }
        //dump('计租面积：'.$roomOne['LeasedArea'].',基价折减率：'.$roomOne['RentPoint'].',结构基价：'.$structureTypePoint.',架空率：'.$emptyPoint.',层次调解率'.$floorPoint."。公式：".'('.$roomOne['LeasedArea'].'*('.$roomOne['RentPoint'].'*'.$structureTypePoint.')'.'*'.$emptyPoint.'*'.$floorPoint.')');exit;
