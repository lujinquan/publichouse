...
//halt($data);
                ...
                //halt($data);
                    ...
                    $datas['Remark'] = $data['batchReason'];
                    $datas['InflRent'] = $data['diff'];
                    $datas['ChangeType'] = 17;  //租金调整（0.5批量）
                    $datas['ProcessConfigName'] = $changeTypes[17];  //异动名称
                    $datas['ProcessConfigType'] = Db::name('process_config')->where(['Status'=>1,'Type'=>17])->order('id desc')->value('id');        //流程控制线路
                    if(!$datas['ProcessConfigType']){
                        return jsons('4001','请先联系超级管理员配置异动流程');
                    }
                    $datas['ChangeOrderID'] = date('YmdHis', time()).'16'.$suffix;   
                ...
                case 18:  // 楼栋注销
                    ...
                    $datas['Remark'] = $data['cancelReason'];  //异动缘由
                    $datas['ChangeType'] = $data['type'];  //异动类型
                    $datas['ProcessConfigName'] = $changeTypes[18];  //异动名称
                    $datas['ChangeImageIDS'] = isset($ChangeImageIDS)?$ChangeImageIDS:'';  //附件集
                    $datas['ProcessConfigType'] = Db::name('process_config')->where(['Status'=>1,'Type'=>18])->order('id desc')->value('id');        //流程控制线路
                    if(!$datas['ProcessConfigType']){
                        return jsons('4001','请先联系超级管理员配置异动流程');
                    }
                    $datas['ChangeOrderID'] = date('YmdHis', time()).'18'.$suffix;   //08代表注销
                    //halt($datas);
                    $res = Db::name('change_order')->insert($datas);
                    break;
