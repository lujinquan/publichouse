 ...
 * @description
 */
class CorrectAudit extends Base
{
    /**
     * @title 使用权变更审核主页
     * @author Mr.Lu
    ...
    public function supply(){
        ...
        if($this->request->isPost()) {
            ...
            if(isset($_FILES) && $_FILES){ //由于目前前端的多文件上传一次只上传一个标题的多张图片，所以目前  $_FILES  只有一个元素，故 $ChangeImageIDS 只是一个字符串
                ...
                $nowStatus = Db::name('cor_change_order')->where('ChangeOrderID' ,'eq' ,$changeOrderID)->value('Status');
            ...
            $oldImageIDS = Db::name('cor_change_order')->where('ChangeOrderID' ,'eq' ,$changeOrderID)->value('ChangeImageIDS');
            ...
            if(isset($changeImageIDS)){
                $effect = Db::name('cor_change_order')->where('ChangeOrderID' ,'eq' ,$changeOrderID)->setField('ChangeImageIDS' ,$changeImageIDS);
            }
            
            if(isset($update)){
                Db::name('cor_change_order')->where('ChangeOrderID' ,'eq' ,$changeOrderID)->update($update);
            }
            //资料补充成功后，做后置操作，修改主订单当前状态，创建子订单，即子订单记录
            
//halt($changeOrderID);
            //生成子订单
            model('ph/CorrectAudit')->create_child_order($changeOrderID);
        ...
        //检查是否允许补充资料
        $checkSupply = model('ph/CorrectAudit')->check_supply($changeOrderID);
        ...
        $data['detail'] = model('ph/CorrectAudit')->get_change_detail_info($changeOrderID);

        $data['config'] = model('ph/CorrectAudit')->process_status($changeOrderID);

        $data['record'] = model('ph/CorrectAudit')->process_record($changeOrderID);
    ...
    public function detail(){
        ...
        $res['detail'] = model('ph/CorrectAudit')->get_change_detail_info($changeOrderID);

        $res['config'] = model('ph/CorrectAudit')->process_status($changeOrderID);

        $res['urls'] = model('ph/CorrectAudit')->process_imgs_url($changeOrderID);

        $res['record'] = model('ph/CorrectAudit')->process_record($changeOrderID);
    ...
    public function process(){
        ...
        if($this->request->isPost()) {
            ...
            model('ph/CorrectAudit')->check_process($data['ChangeOrderID']);

            if(!isset($data['reson'])) $data['reson']='';
       
            $result = model('ph/CorrectAudit')->create_child_order($data['ChangeOrderID'], $data['reson']);
