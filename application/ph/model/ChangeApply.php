    ...
    public function check_apply_table($data)
    {
        ...
        switch ($data['type']) {
                ...
                case 2:
                    ...
                    $houseModel = new HouseModel;
                    $finds = $houseModel->field('InstitutionPID ,InstitutionID,BanID,HousePrerent,TenantID,IfEmpty,OwnerType,UseNature')
                                        ->where($findwhere)
                                        ->find();
                    if(!$finds){
                        return jsons('4002','房屋状态异常');
                ...
                break;
                case 17:
                    $ifin = Db::name('change_order')->where(['BanID' =>['eq' ,$data['banID']],'ChangeType'=>17,'Status'=>['>',1],'OrderDate'=>date('Ym',time())])->find();
                    if($ifin){
                        return jsons('4001','该楼栋已在租金调整（批量）异动单中……');
                    }
                    $banModel = new BanModel;
                ...
                case 18:
                    ...
                    $finds = $banModel->field('TubulationID,InstitutionID,DamageGrade,StructureType,OwnerType,TotalOprice,PreRent,TotalArea')
                                      ->where($findwhere)
                                      ->find();
                    if(!$finds){
                        return jsons('4002','楼栋状态异常');
