...
class CorrectAudit extends Model
{
    // 设置当前模型对应的完整数据表名称
    ...
    public function get_change_detail_info($changeOrderID){
        ...
        //房屋编号，申请类型
        $orderData = self::field('HouseID , TransferRent ,OwnerType,UseNature,ChangeReason,ChangeType, OldTenantID, CreateTime,OldTenantName,NewTenantName,NewCard')->where('ChangeOrderID' ,'eq' ,$changeOrderID)->find();
        ...
        $data['NewTenantName'] = $orderData['NewTenantName'];  //租户姓名
        $data['NewCard'] = $orderData['NewCard'];  //租户身份证号
    ...
    public function create_child_order($changeOrderID,$reson=''){
        ...
        //获取流程总人数
        $total = Db::name('cor_change_order')->alias('a')
        ...
        $step = Db::name('cor_child_order')->where($option)->max('Step');
        ...
        $re = Db::name('cor_child_order')->insert($datas);  //创建子订单

        if($status < $total && $reson != ''){
            Db::name('cor_child_order')->where('FatherOrderID' ,'eq' ,$changeOrderID)->setField('IfValid' ,0); //重置之前的子订单状态为无效
        }
    ...
    public function process_imgs_url($changeOrderID){
        $IDS = Db::name('cor_change_order')->where('ChangeOrderID' ,'eq' ,$changeOrderID)->value('ChangeImageIDS');
        $images = explode(',' ,$IDS);
...
//halt($changeOrderID);
        $process = Db::name('cor_change_order')->alias('a')
                                               ->join('process_config b' ,'a.ProcessConfigType = b.Type' ,'left')
                                               ->where('a.ChangeOrderID' ,'eq' ,$changeOrderID)
    ...
    public function process_record($changeOrderID){
        ...
        $record = Db::name('cor_child_order')->where('FatherOrderID' ,'eq' ,$changeOrderID)
...
//halt($record);
    ...
    public function check_process($changeOrderID){
        ...
        $one =  Db::name('cor_change_order')->alias('a')
                                            ->join('process_config b' ,'a.ProcessConfigType = b.Type' ,'left')
                                            ->where('a.ChangeOrderID' ,'eq' ,$changeOrderID)
                                            ->field('a.Status ,b.id ')
                                            ->find();
