...
function getCode(obj){
    ...
    if($.trim(telNum) == ''){
    	...
    	objtip.hide();
        $.post('/user/Publics/forgetpass', 'telNum='+telNum, function(res){
...
function sendCode(){
    ...
    if($.trim(telNum) == ''){
	    ...
	    $.post('/user/Publics/forgetpass', 'telNum='+telNum+'&code='+code, function(result){
	        if (result.code ==1) {
	            setCookie('ph_code_sceond_down', 0, 1);
	            objtip.html('');
	            objtip.css('color', 'blue');
	            objtip.show();
	            //objtip.html("验证成功 正在跳转...");
