...
$results=$mysqli->query("select HouseID,TenantName,HousePrerent,BanAddress from ph_house where InstitutionID = $institution and OwnerType = $ownerType");
...
//  	$mysqli->query("update $table set HouseID = '".$res['HouseID']."' where TenantName = '".$res['TenantName']."'");
// }exit;
...
//获取报表中的所有匹配到房屋编号的数据
$result=$mysqli->query("select HouseID,TenantName,HousePrerent,BanAddress from $table where HouseID != ''");
// $result = $result->fetch_all(MYSQLI_ASSOC);
// halt($result);

//获取报表中的所有未匹配到房屋编号的数据
$result2=$mysqli->query("select TenantName,HousePrerent,BanAddress from $table where HouseID =''");
