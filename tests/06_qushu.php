...
$results=$mysqli->query("select HouseID,TenantName,HousePrerent,BanAddress from ph_house where InstitutionID = 6 and OwnerType = 2");
...
//  	$mysqli->query("update old_house_06qu set HouseID = '".$res['HouseID']."' where TenantName = '".$res['TenantName']."'");
// }exit;
...
//获取报表中的所有匹配到房屋编号的数据
$result=$mysqli->query("select HouseID,TenantName,HousePrerent,BanAddress from old_house_06qu where HouseID != ''");
// $result = $result->fetch_all(MYSQLI_ASSOC);
// halt($result);

//获取报表中的所有未匹配到房屋编号的数据
$result2=$mysqli->query("select TenantName,HousePrerent,BanAddress from old_house_06qu where HouseID =''");
